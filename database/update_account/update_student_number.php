<?php
	...
	if($db_found){
		...
		if($student_number == mysqli_real_escape_string($db_found, $_SESSION['student_id'])){
			...
			if($result){
				...
				$SQL = "SELECT student_id FROM tbl_users WHERE ID='".mysqli_real_escape_string($db_found, $_SESSION['ID'])."'";
				$result = mysqli_query($db_found, $SQL);
					
				if($result){
					$row = mysqli_fetch_assoc($result); 
					$_SESSION['student_id'] = mysqli_real_escape_string($db_found, $row['student_id']);
					print "success";	
				}
				else{
					print "failure1";
